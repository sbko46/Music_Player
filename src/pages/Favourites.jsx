...
export default function Favourites({
  ...
  return (
    <div className="p-4 text-white bg-gradient-to-br from-[#3a1c0e] to-[#2a0900] min-h-screen">
      <h2 className="text-2xl font-bold mb-10"> Favourites</h2>

      {songs.length === 0 ? (
        <p className="text-gray-300">No favourite songs yet.</p>
      ) : (
        <ul className="space-y-2">
          {songs.map((song, index) => (
            <li
              key={index}
              onClick={() => {
                setCurrentSongIndex(song.index); // Must be passed correctly while storing favourites
                setIsPlaying(true);
              }}
